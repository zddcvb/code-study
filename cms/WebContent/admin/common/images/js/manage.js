manage = {
  ...
  tajaxLoads: function(_strers, _strid)
  {
    ...
    if (manage.tckBackString(tstrers)) setInnerHTML($I(tstrid), tstrers);
  },
...
manage.contextmenu = {
    ...
    if (tobj)
    {
      ...
      for(ti = 0; ti < tliobj.length; ti++)
      {
        try
        {
          ...
          if (tliulobj) tliobj[ti].innerHTML += $I('MenuMoreContent').value;
        } catch(e){};
  ...
  tLoad: function(_name, _event, _argkey)
  {
    ...
    if (manage.contextmenu.tnName == null)
    {
      ...
      var tobj = $I('ContextMenu');
      var tobjct = $I('ContextMenuContent-' + tname);
      if (tobj && tobjct)
      {
...
manage.desktop = {
  ...
  tAdd: function(_name, _topic, _icon, _link)
  {
    ...
    if (tobj && !tlnkobj)
    {
      ...
      var tinnerHTML = $I('DesktopLnkContent').value;
      ...
      $I(tlnkname).innerHTML = tinnerHTML;
    };
  ...
  {
    var tname = _name;
    var tobj1 = $I(tname + '-windowsTitleText');
  ...
  {
    var tstrname = _strname;
    var tobj = $I(tstrname + '-Topic');
    if (tobj)
    {
      var tinputID = tstrname + '-Topic-Input';
      var tobjpt = $I(tinputID);
      if (!tobjpt)
      {
  ...
  tCtmRenameIt: function(_strname, _strvalue)
  {
    ...
    var tobj1 = $I(tstrname + '-Topic');
    var tobj2 = $I(tstrname + '-Topic-Input');
    tstrvalue = tstrvalue.replace(/(^\s*)|(\s*$)/g, '');
    if (tobj1 && tobj2)
    {
      if (!tstrvalue)
      {
...
manage.history = {
  ...
  tSetHistoryImg: function()
  {
    var tnhAry = manage.history.thAry;
    var tnnIndex = manage.history.tnIndex;
    var tBackImgObj = $I('History-Img-Back');
    var tForwardImgObj = $I('History-Img-Forward');
...
manage.login = {
  tLoginClick: function()
  {
    ...
    var tstrform = 'username=' + iescape($I('username').value) + '&password=' + iescape($I('password').value) + '&valcode=' + iescape($I('valcode').value);
  ...
  tLoginClicks: function(_strers)
  {
    ...
    var tstate = cls.tgetParameter(tstrers, 'state');
    switch (tstate)
    {
      case '0':
        manage.windows.dialog.tAlert($I('Message_Login_0').value);
        break;
      case '1':
        manage.windows.dialog.tAlert($I('Message_Login_1').value);
        break;
      case '200':
        manage.tInit();
        break;
...
manage.menubar = {
  tAdd: function(_name)
  {
    ...
    if (tobj && !tobjbar)
    {
      ...
      if ($I('MenuBarDIV').innerHTML == '&nbsp;') $I('MenuBarDIV').innerHTML = '';
      $I('MenuBarDIV').appendChild(tDiv);
      $I(tnamebar).innerHTML = $I(tname + '-windowsTitleText').innerHTML;
      manage.menubar.tChecked(tname);
    };
  },
  tChecked: function(_name)
  {
    ...
    var tobj = $I(tname);
    var tnamebar = 'MenuBarDIV-' + tname;
    var tobjbars = $I('MenuBarDIV').getElementsByTagName('div');
  ...
  tRemove: function(_name)
  {
    ...
    var tnamebar = 'MenuBarDIV-' + tname;
    var tobjbar = $I(tnamebar);
    if (tobjbar) $I('MenuBarDIV').removeChild(tobjbar);
    if ($I('MenuBarDIV').innerHTML == '') $I('MenuBarDIV').innerHTML = '&nbsp;';
  },
  tDesktop: function()
  {
...
manage.windows = {
  ...
  tgetzIndex: function(_id)
  {
    ...
    if (tobj)
    {
      ...
      {
        twtobj = $I(manage.windows.tzIndexObj.id + '-windowsTitle');
	...
	tAdd: function(_name, _title, _url, _width, _height)
  {
    ...
    else
    {
      ...
      $I('body').appendChild(tDiv);
      var tinnerHTML = $I('windowsContent').value;
      tinnerHTML = tinnerHTML.replace(/(\{\$id\})/g, tname);
      tinnerHTML = tinnerHTML.replace(/(\{\$title\})/g, ttitle);
      $I(tname).innerHTML = tinnerHTML;
      $I(tname).style.zIndex = manage.windows.tgetzIndex(tname);
      $I(tname + '-windowsIframe').src = turl;
  ...
  {
    var tname = _name;
    if ($I(tname)) $I(tname).style.zIndex = manage.windows.tgetzIndex(tname);
  },
  ...
  tRemove: function(_name)
  {
    ...
    if (tobj)
    {
      var tidobj = $I(tname + '-windowsIframeDIV');
      var tiobj = $I(tname + '-windowsIframe');
      if (tidobj && tiobj) tidobj.removeChild(tiobj);
      $I('body').removeChild(tobj);
  ...
  tMax: function(_name)
  {
    ...
    if (tobj)
    {
      ...
      else
      {
        ...
        tobj.style.height = (cls.doc.tclientHeight() - $I('Menu').clientHeight - 2) + 'px';
      };
    };
    manage.windows.tResizeIframe(tname);
  },
  ...
  tRemoveShade: function(_obj)
  {
    ...
    var tpobj = tobj.parentNode;
    var tname = tpobj.id + '-windowsShade';
    if ($I(tname)) tobj.removeChild($I(tname));
  },
  tResize : function()
  {
  ...
  tResizeShade: function(_name)
  {
    var tname = _name;
    var tpobj = $I(tname);
    var tsobj = $I(tname + '-windowsShade');
    var ttobj = $I(tname + '-windowsTitle');
  ...
  tResizeIframe: function(_name)
  {
    var tname = _name;
    var tpobj = $I(tname);
    var tobj = $I(tname + '-windowsTitle');
    var tfobj = $I(tname + '-windowsIframe');
    if (tpobj && tobj && tfobj) tfobj.style.height = (tpobj.clientHeight - tobj.clientHeight) + 'px';
  },
  tResizePanel: function()
  {
    var ttobj = $I('windowsContent-topPanel');
    var tlobj = $I('windowsContent-leftPanel');
    var trobj = $I('windowsContent-rightPanel');
    ...
    var trcobj = $I('windowsContent-rightPanel-Content');
    if (ttobj && trcobj)
    {
      var tmobj = $I('windowsContent-Menu');
  ...
  {
    var tobj = $I('windowsContent-Panel-Loading');
    ...
    if (manage.windows.resize.tdobj == tobj)
    {
      if (manage.windows.resize.tstate == 0)
      {
        try
        {
          ...
          $I(tobj.parentNode.id).className = 'windowsT';
          $I(tobj.parentNode.id + '-windowsShade').style.display = 'none';
          $I(tobj.parentNode.id + '-windowsIframeDIV').style.display = 'none';
        } catch (e) {};
      };
      var tx = cls.mouse.tx(tevent);
      var ty = cls.mouse.ty(tevent);
  ...
  tstop: function(_obj)
  {
    ...
    if (manage.windows.resize.tdobj == tobj)
    {
      ...
      try
      {
        ...
        $I(tobj.parentNode.id).className = 'windows';
        $I(tobj.parentNode.id + '-windowsShade').style.display = 'block';
        $I(tobj.parentNode.id + '-windowsIframeDIV').style.display = 'block';
