...
manages.options = {
  ...
  tAdd: function()
  {
    var tnumObj = $I('optionsnum');
    var tstrURL = manages.tinterfaceURL;
    manages.options.tcount = cls.tgetNum(tnumObj.value, 0);
    tstrURL += '?type=options&utype=add&id=' + manages.options.tcount;
    manages.options.tcount += 1;
    tnumObj.value = manages.options.tcount;
    manage.windows.tShowPanelLoading();
    manage.tajaxGet(tstrURL, manages.options.tAdds);
  },
  tAdds: function(_strers)
  {
    var tstrers = _strers;
    ...
    if (manage.tckBackString(tstrers))
    {
      $I('optionsListFactory').innerHTML = tstrers;
      var tnewoptionsObj = $I('optionsListFactory').getElementsByTagName('table')[0];
      if (tnewoptionsObj) $I('optionsList').appendChild(tnewoptionsObj);
    };
  },
  tEdit: function(_strfid)
  {
    tstrfid = _strfid;
    var tstrURL = manages.tinterfaceURL;
    tstrURL += '?type=options&utype=edit&fid=' + tstrfid;
    manage.windows.tShowPanelLoading();
    manage.tajaxGet(tstrURL, manages.options.tEdits);
  },
  tEdits: function(_strers)
  {
    var tstrers = _strers;
    ...
    if (manage.tckBackString(tstrers))
    {
      var tAry = tstrers.split('options.tDelete');
      $I('optionsnum').value = tAry.length;
      $I('optionsList').innerHTML = tstrers;
    };
  },
  tDelete: function(_id)
  {
    var tid = _id;
    var tobj = $I('options-' + tid);
