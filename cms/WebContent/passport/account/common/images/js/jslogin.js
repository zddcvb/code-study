jslogin = {
  ...
  tCkLogin: function(_strid, _strbaseURL)
  {
    ...
    var tobj1 = $I(jslogin.tid);
    var tobj2 = $I(jslogin.tid + '-Message-1');
    if (tobj1 && tobj2) tobj1.innerHTML = tobj2.value;
    cls.tigets(jslogin.tbaseURL + '/interface.jsp?type=cklogin&id=' + jslogin.tid, jslogin.tCKLogins);
  },
  tCKLogins: function(_strers)
  {
    var tstrers = _strers;
    if (wfront.tckBackString(tstrers)) $I(jslogin.tid).innerHTML = tstrers;
  },
  tLogin: function(_strfname)
  {
    ...
    if (tformObj)
    {
      ...
      var tobj1 = $I(jslogin.tid);
      var tobj2 = $I(jslogin.tid + '-Message-2');
      if (tobj1 && tobj2) tobj1.innerHTML = tobj2.value;
    };
    return false;
  },
  tLogins: function(_strers)
  {
    ...
    if (tstrers == '-101')
    {
      var tobj1 = $I(jslogin.tid + '-Message-4');
      if (tobj1) terrorstring = tobj1.value;
    };
    if (tstrers == '-102')
    {
      var tobj2 = $I(jslogin.tid + '-Message-5');
  ...
  {
    var tobj1 = $I(jslogin.tid);
    var tobj2 = $I(jslogin.tid + '-Message-3');
    if (tobj1 && tobj2) tobj1.innerHTML = tobj2.value;
    cls.tigets(jslogin.tbaseURL + '/interface.jsp?type=action&atype=logout', jslogin.tLogouts);
  },
