defaults = {
  ...
  tVote: function(_strfname)
  {
    ...
    if (tformObj)
    {
      ...
      if (wfront.validator.tCheck(tformObj))
      {
        ...
        $I('ajaxVoteSubmit').disabled = true;
  ...
  {
    var tstrers = _strers;
    $I('ajaxVoteSubmit').disabled = false;
...
defaults.votes = {
  ...
  tAdds: function(_strers)
  {
    ...
    if (wfront.tckBackString(tstrers))
    {
      $I('votesListFactory').innerHTML = tstrers;
      var tnewurlsObj = $I('votesListFactory').getElementsByTagName('table')[0];
      if (tnewurlsObj) $I('votesList').appendChild(tnewurlsObj);
    };
  },
  tDelete: function(_id)
  {
    var tid = _id;
    var tobj = $I('votes-' + tid);
